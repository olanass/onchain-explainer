...
async function browserFixture({ account = creator, existing = false, category = 'Data' } = {}) {
  ...
  const service = { name: 'Olanas Onchain Explainer', category, slug: 'olanas-onchain-explainer', price: '10', currency: 'OLANAS', gatewayUrl: 'https://olanas.xyz/x402/olanas-onchain-explainer' };
...
test('published listing update signs only the category and preserves payment terms', async () => {
  const fixture = await browserFixture({ existing: true, category: 'Developer Tools' });
  assert.equal(fixture.element('publish').textContent, 'Sign category update');
  await fixture.element('publish').events.click();
  const call = fixture.calls.find(call => call.options?.method === 'PATCH');
  const posted = JSON.parse(call.options.body);
  assert.deepEqual(posted.changes, { category: 'Data' });
